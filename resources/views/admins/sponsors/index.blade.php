<x-main-layout title="1000 أمل لأبناء الشهداء">
    ...
    <div class="header fw-bold" style="color: var(--primary-color);font-size:18px;  border-top-right-radius: 6px;border-top-left-radius:6px; padding:10px;background:linear-gradient(to left , #c6fdda , #edfaf1)">
        ...
        <p class="color-text fw-normal mt-2" style="font-size: 15px">
            يعرض هذا القسم الكافلين المسجلين في النظام، حيث يمكنك الاطلاع على ملفاتهم الشخصية، مراجعة بياناتهم، وتحديثها عند الحاجة.  مما يسهل عملية الإشراف والإدارة بشكل شامل ومنظم.
        </p>
    </div>
        ...
        <div class="rounded">
            ...
            <div class="mt-4">
                <div class="d-flex justify-content-between mb-3">
                    <p class="fs-5 fw-semibold"> قائمة الكفلاء في النظام </p>
                    <a href="{{route('admin.sponsor.create')}}" class="submit-btn text-decoration-none">+ إضافة كافل </a>
                </div>

                <form action="{{route('admin.sponsor.index')}}" method="GET" class="search custom-sm-style w-100">

                    @csrf
                    <div class="input-group flex-nowrap mb-4">

                        <input type="text" name="search" class="form-control" placeholder="{{__('البحث عن كافل...')}}"  aria-describedby="addon-wrapping">
                ...
                <table  class=" border-0 w-100 text-center" style="border-collapse: collapse;">
                    ...
                    <tbody>
                        ...
                        @forelse ($sponsors as $sponsor)
                            ...
                            <tr>
                                <td> <span class="value"> {{$sponsor->id}}     </span> </td>
                                <td> <span class="value"> {{$sponsor->name}}     </span> </td>
                                <td><span class="value">  {{$sponsor->phone}}    </span></td>
                                <td><span class="value">  {{$sponsor->email}}    </span></td>
                                <td><span class="value">  {{$sponsor->country}}  </span></td>
                                {{-- <td><span class="value">  {{$association->phone}}                 </span></td> --}}
                                <td style="position: relative;">

                                    <img class="show-action" src="{{asset('images/Group 8.svg')}}" alt="">

                                    <div class="action" >
                                        <a href="{{route('admin.sponsor.show' , $sponsor->id)}}" class="text-decoration-none mb-1">
                                            <img src="{{asset('images/Show.svg')}}" alt="">
                                            <span style="color: var(--text-color);">{{__('عرض التفاصيل')}}</span>
                                        </a>


                                        <a href="{{route('admin.sponsor.edit' , $sponsor->id)}}" class="text-decoration-none mb-1" style="gap: 10px">
                                        ...
                                        <button  class="submit border-0 p-0 bg-transparent d-flex btn-delete" style="gap:7px">
                                            ...
                                            <span style="color: var(--text-color);">{{__(' حذف الكفيل ')}}</span>
                                        </button>

                                        <form  action="{{route('admin.sponsor.destroy' , $sponsor->id)}}" method="post" style="display: none">
                                            @csrf
                                            @method('delete')
                        ...
                        @empty
                             ...
                             <tr>
                                <td colspan="6" class="text-center fs-5" style="color:var(--primary-color)">
                                    {{__('لا يوجد كفلاء مسجلين في النظام')}}
                                </td>
                            </tr>
